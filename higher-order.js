...
//Code Here
let totalPopulation = populations.reduce((rt, elem) => rt + elem)
...
let orderTotals = orders.map(elem => elem.price * elem.tax + elem.price)
...
let bobsTotal = purchases.filter(elem => elem.owner === "Bob").map(elem => elem.price).reduce((ct, elem) => ct + elem) 
